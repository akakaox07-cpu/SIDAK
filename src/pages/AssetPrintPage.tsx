...
// Printable A4 layout for an Asset
const AssetPrintPage: React.FC<Props> = ({ asset }) => {
  ...
  // Mark ready when QR is generated and image loaded (or no image)
  useEffect(() => {
    const hasImage = Boolean(asset.photos?.[0]);
    if (qrLink && (!hasImage || imageLoaded)) {
  ...
  return (
    <div className="min-h-screen bg-gray-300 p-8 print:p-0">
      {/* Top-right controls: signature tools + print button, shown side by side (not printed) */}
      <div className="fixed top-6 right-6 z-50 print:hidden">
        {/* Tools column with compact header row; avoids unused width when collapsed */}
      ...
      {/* Loading overlay - hidden when printing */}
      {!readyToPrint && (
        <div className="fixed inset-0 bg-white bg-opacity-90 flex items-center justify-center z-50 print:hidden">
          <div className="text-center">
            ...
            <div className="text-sm text-gray-600">Memuat halaman cetak...</div>
          </div>
        </div>
      )}
      ...
      <div className="mx-auto bg-white w-[210mm] min-h-[297mm] shadow-lg rounded-lg p-12 box-border print:shadow-none print:rounded-none print:p-6">
        <h1 className="text-center text-xl font-semibold mb-4 print:mb-2">Detail Aset</h1>
        ...
        <div className="border rounded-lg p-5 bg-gray-50 mb-6 print:p-3 print:mb-4">
          <div className="grid grid-cols-[160px_1fr] gap-y-2 text-sm">
            ...
            {asset.luasTanah && <><div className="font-semibold">Luas Tanah</div><div>{asset.luasTanah}</div></>}
            {asset.luasBangunan && <><div className="font-semibold">Luas Bangunan</div><div>{asset.luasBangunan}</div></>}
        ...
        <table className="w-full text-sm mt-8 print:mt-4">
          <tbody>
            ...
            {/* Area tanda tangan (spacer) — diperbesar agar area tanda tangan lebih lega saat dicetak */}
            <tr>
