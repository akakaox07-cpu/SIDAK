...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-blue-500 via-blue-600 to-purple-700 flex items-center justify-center p-4" style={{ position: 'relative', zIndex: 1 }}>
      <div className="w-full max-w-5xl" style={{ position: 'relative', zIndex: 100 }}>
        ...
        <div className="bg-white overflow-hidden grid md:grid-cols-2" style={{ position: 'relative', zIndex: 101 }}>
          ...
          <div className="px-8 py-8">
            <div className="mb-6">
              <div className="w-16 h-16 bg-linear-to-br from-blue-600 to-purple-600 rounded-xl flex items-center justify-center mb-4 shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-linear-to-r from-blue-600 to-purple-600 text-white font-semibold py-2.5 px-4 rounded-lg hover:from-blue-700 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm"
              >
                {isLoading ? (
                  <>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowForgot(false)} className="text-gray-500 hover:text-gray-700">✕</button>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={fpUsername} onChange={e=>setFpUsername(e.target.value)} className="w-full input-style" placeholder="username Anda" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Kontak (WA/Email)</label>
                <input value={fpContact} onChange={e=>setFpContact(e.target.value)} className="w-full input-style" placeholder="08xx... / email" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Pesan</label>
                <textarea value={fpMessage} onChange={e=>setFpMessage(e.target.value)} className="w-full input-style" rows={3} placeholder="Keterangan tambahan (opsional)" />
              </div>
            </div>
            <div className="mt-5 flex justify-end gap-2">
              <button onClick={() => setShowForgot(false)} className="px-3 py-2 text-sm rounded-lg bg-gray-100 hover:bg-gray-200">Batal</button>
              ...
              }} className="px-3 py-2 text-sm rounded-lg bg-blue-600 text-white hover:bg-blue-700">Kirim ke Admin</button>
            </div>
          </div>
        </div>
      )}
